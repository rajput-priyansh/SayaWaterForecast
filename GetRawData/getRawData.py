...
def startWaterForecastFileDownload():
    try:
        ...
        #sDate = datetime.datetime(2022, 4, 1)
        sDate = datetime.today().replace(day=1)+  relativedelta(months = -6)
        ...
        #eDate =datetime.datetime(2022, 10,31)
        eDate = datetime.today().replace(day=1) + timedelta(days=-1)  #end date is last month's last date
        ...
        connObj = connectionDB.connectDatabase("SayaLive", "Mli$3120$8%(0",
                                               ...
                                               "SAYA")
        cur = connObj.cursor()
        customerIdQueryFor6MonthData="public.\"getcustomerIdForForecasttable\""
        ...
        #finalIds = [10821,10941,10940,10942,10881,10943,10822,10823,10824,10840,10870,10873,10830,10837,10880,10857,10884,10874,10853,10877,10878,10848,10876,10834,10846,10847,10859,10850,10826,10839,10879,10842,10861,10838,10858,10864,10854,10841,10835,10939,10852,10845,10865,10855,10866,10882,10827,10938,10828,10863,10832,10849,10883,10836,10862,10843,10868,10833,10825,10867,10871,10856,10875,10860,10869,10831,10851,10872,10829,10844]
        finalIds = [10859]
        #todaycomment with connObj.cursor() as cur:
        #todaycomment     cur.callproc(customerIdQueryFor6MonthData)
        #todaycomment     #cur.execute(customerIdQueryFor6MonthData)
        #todaycomment     #finalIds = cur.fetchall()
        #todaycomment     print("Fetched records: %s" % finalIds)
        for id in finalIds:
            try:
                ...
                LoggeR.writeExpceptionToTexFile(df.__str__()+"For CustomerId::>"+id.__str__())
                ...
                LoggeR.writeExpceptionToTexFile("Done ForecastData Insert for the Month:-->"+ datetime.now().month.__str__())				   
